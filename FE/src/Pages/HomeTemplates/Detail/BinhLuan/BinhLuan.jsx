...
import { deleteComment, getAllCommentsImage, postComment } from '../../../../Utils/apiUtils'
...
export default function BinhLuan() {
    ...
    useEffect(() => {
        getAllCommentsImage(params.image_id)
            .then(rs => {
                // console.log(rs);
                setComment(rs);
            })
    ...
    const renderComments = () => {
        return comment?.map((cmt, index) => {
            return <article key={index} className="p-6 text-base bg-white border-t border-gray-200 dark:border-gray-700 dark:bg-gray-900">
                <footer className="flex justify-between items-center mb-2">
                    <div className="flex items-center">
                        <p className="inline-flex items-center mr-3 text-sm text-gray-900 dark:text-white font-semibold"><img className="mr-2 w-6 h-6 rounded-full" src="https://flowbite.com/docs/images/people/profile-picture-4.jpg" alt="Helene Engels" />{cmt?.user.full_name}</p>
                ...
                <p className="text-gray-500 dark:text-gray-400">{cmt?.content}</p>
                <div className="flex items-center mt-4 space-x-4">
                    ...
                    <i className="fa fa-trash cursor-pointer" onClick={() => {
                        deleteComment(cmt?.comment_id);
                        getAllCommentsImage(params.image_id)
                        .then(rs => {
                            // console.log(rs);
                            setComment(rs);
        ...
        <section className="bg-white dark:bg-gray-900 py-8 antialiased">
            <div className="max-w-2xl mx-auto px-4">
                ...
                <form className="mb-6">
                    ...
                    <button type="button" className="inline-flex items-center py-2.5 px-4 text-xs font-medium text-center text-white bg-yellow-500 rounded-lg focus:ring-4 focus:ring-primary-200 dark:focus:ring-primary-900 hover:bg-primary-800" onClick={() => {
                        const content = document.querySelector('#comment').value;
                        const formData = {
                            "content": content,
                        }
                        postComment(params.image_id, formData)
                            .then(rs => {
                                // console.log(rs);
                                getAllCommentsImage(params.image_id)
                                    .then(rs => {
